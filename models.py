...
class DiscreteActor(nn.Module):
    ...
    def forward(self, x, block, deterministic=False, q_mask=None, tsallis=False, q_prime=1.2):
        B0, _, H, W = x.size()
        if self.add_coords is not None:
            x = self.add_coords(x)
        ...
        if q_mask is not None:
            q_mask = torch.tensor(q_mask, dtype=torch.float).view(action_logits.shape).to(x.device)
