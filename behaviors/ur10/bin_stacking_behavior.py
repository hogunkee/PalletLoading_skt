...
from isaac_utils import get_bin_info, bbox_info, scaled_bbox_info
...
class BinStackingContext(ObstacleMonitorContext):
    def __init__(self, robot, agent, args=None):
        super().__init__()
        self.robot = robot
    ...
    def stacked_bin_observation(self):
        ...
        stacked_volume = np.mean(self.planner.state)
...
def make_decider_network(robot, agent, args=None):
    return DfNetwork(Dispatch(), context=BinStackingContext(robot, agent, args))
