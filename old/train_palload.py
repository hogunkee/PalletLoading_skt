...
def get_action(env, fc_qnet, state, block, epsilon, crop_min=0, crop_max=64, pre_action=None, with_q=False, deterministic=True):
    ...
    if np.random.random() < epsilon:
        ...
        if deterministic:
            use_mask = True
            if use_mask:
                mask = generate_mask(state, block, q)
                q *= mask
            ...
            use_projection = True
            if use_projection:
                y_mod, x_mod = action_projection(state, block, [aidx_th, aidx_y, aidx_x])
                aidx_y -= y_mod
                aidx_x -= x_mod
        else:
            soft_tmp = 1e-1
            ...
            use_mask = True
            if use_mask:
                mask = generate_mask(state, block, q).reshape((-1,))
                q_probs = q_probs*mask + 1e-4
                q_probs = q_probs / q_probs.sum()
            ...
            use_projection = True
            p_proj = 1.0 if use_projection else 0.0

            if np.random.random() < p_proj:
                y_mod, x_mod = action_projection(state, block, [aidx_th, aidx_y, aidx_x])
                aidx_y -= y_mod
                aidx_x -= x_mod
...
def generate_mask(state, block, q):
    mask = np.ones_like(q)
...
def action_projection(state, block, action):
    action_rot = action[0]
    ...
    for i in reversed(range(max_level)):
        if i == max_level-1:
            ...
            cum_state[i] = np.clip(np.copy(state[i])+np.copy(cum_state[i+1]), 1.0, 0.0)

    level_map = np.sum(cum_state, axis=0)
    box_level = np.max(level_map[max(min_y,0):max_y,max(min_x,0):max_x]) + 1
    if box_level > max_level:
        return 0, 0

    curr_state = state[int(box_level)-1]

    y_ = 0
    while True:
        min_y_, max_y_ = min_y-y_, max_y-y_
        if min_y_ < 0:
            break

        box_placed = np.zeros(np.shape(state[0]))
        box_placed[max(min_y_,0): max_y_, max(min_x,0): max_x] = 1
        curr_map = curr_state + box_placed        
        if len(np.where(np.array(curr_map)>1)[0]) > 0:
            break
        y_ += 1
    if y_ > 0: y_ -= 1
    ...
    while True:
        min_y_, max_y_ = min_y-y_, max_y-y_
        min_x_, max_x_ = min_x-x_, max_x-x_
        if min_x_ < 0:
            break

        box_placed = np.zeros(np.shape(state[0]))
        box_placed[max(min_y_,0): max_y_, max(min_x_,0): max_x_] = 1
        curr_map = curr_state + box_placed        
        if len(np.where(np.array(curr_map)>1)[0]) > 0:
            break
        x_ += 1
...
if __name__=='__main__':
    ...
    parser.add_argument("--model_path", default="1113_1606", type=str)
