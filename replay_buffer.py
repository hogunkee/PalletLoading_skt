...
class OffReplayBuffer(object):
    def __init__(self, state_dim, block_dim=2, max_size=int(5e5), dim_reward=1, dim_action=2):
        ...
        self.qmask = np.zeros([max_size,2] + list(state_dim)[1:], dtype=np.uint8)
    ...
    def add(self, state, block, qmask, action, next_state, next_block, next_qmask, reward, done):
        self.state[self.ptr] = np.array(state, dtype=np.uint8)
        self.block[self.ptr] = block 
        self.qmask[self.ptr] = np.array(qmask, dtype=np.uint8)
    ...
    def sample(self, batch_size):
        ...
        data_batch = [
            ...
            torch.FloatTensor(self.qmask[ind]).to(self.device),
...
class OnReplayBuffer(object):
    def __init__(self, dimS, dimA, gamma, lam, lim):
    ...
    def add(self, state, block, action, reward, value, prob, n_qmask):
